...
struct ans_Files {
	...
	ans_Files(String s) {
		...
		id = StrToInt(tree->GetValue("id")->Value());
	};
...
struct SSearch {
	...
	SSearch(String s) {
		...
		size = req->GetValue("size")->Value().c_str();
		dateC = req->GetValue("dateC")->Value().c_str();
	};
	...
	SSearch(String b, String c, String d, String e) : path(b), reg(c), size(d),
		dateC(e) {
	};
